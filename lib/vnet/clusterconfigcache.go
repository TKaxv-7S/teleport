...
import (
	...
	vnetapi "github.com/gravitational/teleport/api/types/vnet"
)
...
func (c *ClusterConfigCache) getClusterConfigUncached(ctx context.Context, clusterClient ClusterClient) (*ClusterConfig, error) {
	...
	ipv4CIDRRange := vnetapi.DefaultIPv4CIDRRange
	...
	if trace.IsNotFound(err) || trace.IsNotImplemented(err) {
		...
		ipv4CIDRRange = cmp.Or(vnetConfig.GetSpec().GetIpv4CidrRange(), vnetapi.DefaultIPv4CIDRRange)
	}
